...
def render_codeforces_performance_graph(daily_df, metric_col="productive_no_test_hours", metric_label="Productive Hours (Excl. Test)"):
    ...
    # Guard: Strictly exclude current day and future dates — no rating should be based on current day
    plot_df = plot_df[plot_df['date_dt'].dt.date < today]
    plot_df = plot_df.sort_values('date_dt').reset_index(drop=True)

    if plot_df.empty:
        st.info("No completed days available yet. Performance ratings and tiers are evaluated on completed days.")
        return
    ...
    # Rating Header: Always show previous day with relevant data logged
    # For productivity rating: previous day with study or waste logged
    # For study rating: previous day with study data logged
    completed_df = plot_df[plot_df['date_dt'].dt.date < today]
    logged_mask = pd.Series(False, index=completed_df.index)
    
    if is_pct:
        # Productivity Rating: study or waste data logged
        if 'productive_hours' in completed_df.columns:
            logged_mask |= (completed_df['productive_hours'].fillna(0) > 0)
        if 'study_hours' in completed_df.columns:
            logged_mask |= (completed_df['study_hours'].fillna(0) > 0)
        if 'waste_hours' in completed_df.columns:
            logged_mask |= (completed_df['waste_hours'].fillna(0) > 0)
        if 'productivity_%' in completed_df.columns:
            logged_mask |= (completed_df['productivity_%'].fillna(0) > 0)
        if 'waste_%' in completed_df.columns:
            logged_mask |= (completed_df['waste_%'].fillna(0) > 0)
    else:
        # Study / Hours Rating: study data logged
        if 'study_hours' in completed_df.columns:
            logged_mask |= (completed_df['study_hours'].fillna(0) > 0)
        if 'productive_hours' in completed_df.columns:
            logged_mask |= (completed_df['productive_hours'].fillna(0) > 0)
        if 'productive_no_test_hours' in completed_df.columns:
            logged_mask |= (completed_df['productive_no_test_hours'].fillna(0) > 0)
        if metric_col in completed_df.columns:
            logged_mask |= (completed_df[metric_col].fillna(0) > 0)

    if not logged_mask.any() and metric_col in completed_df.columns:
        logged_mask |= (completed_df[metric_col].fillna(0) > 0)

    logged_df = completed_df[logged_mask]
    if not logged_df.empty:
        curr_val = float(logged_df[metric_col].fillna(0).iloc[-1])
        curr_date = str(logged_df['date'].iloc[-1])
        yesterday = today - timedelta(days=1)
        try:
            curr_d = pd.to_datetime(curr_date).date()
            rating_day_label = "Previous Day" if curr_d == yesterday else "Previous Logged Day"
        except:
            rating_day_label = "Previous Day"
    ...
    rating_banner_html = (
        ...
        f'<div>'
        f'<div style="font-size:11px;color:#94a3b8;text-transform:uppercase;letter-spacing:0.06em;font-weight:600;">Current Rating ({rating_day_label})</div>'
...
def calculate_cf_stats(df, activity_type="All", year=None):
    ...
    if df.empty:
        return {
            ...
            "all_time_days": 0, "year_days": 0, "month_days": 0,
    ...
    elif activity_type in ("Walk/Exercise", "Walk", "Exercise"):
        act_df = work_df[work_df['type'].isin(['Walk/Exercise', 'Walk', 'Exercise'])].copy()
...
def render_codeforces_stats_cards(stats, unit="hours"):
    ...
    cards_html = (
        ...
        f'<div style="display:flex;flex-direction:column;"><div style="font-size:26px;font-weight:700;color:#f8fafc;line-height:1.15;letter-spacing:-0.02em;">{stats["all_time_hrs"]:g} {unit}</div><div style="font-size:13px;color:#94a3b8;margin-top:5px;">all-time ({stats.get("all_time_days", 0)} active days)</div></div>'
        f'<div style="display:flex;flex-direction:column;"><div style="font-size:26px;font-weight:700;color:#f8fafc;line-height:1.15;letter-spacing:-0.02em;">{stats["year_hrs"]:g} {unit}</div><div style="font-size:13px;color:#94a3b8;margin-top:5px;">in {yr} ({stats.get("year_days", 0)} active days)</div></div>'
        f'<div style="display:flex;flex-direction:column;"><div style="font-size:26px;font-weight:700;color:#f8fafc;line-height:1.15;letter-spacing:-0.02em;">{stats["month_hrs"]:g} {unit}</div><div style="font-size:13px;color:#94a3b8;margin-top:5px;">last 30 days ({stats.get("month_days", 0)} active days)</div></div>'
        f'</div>'
...
def _build_heatmap_grid(df, activity_type, year, today):
    ...
    if not work_df.empty:
        ...
        if activity_type == "Study":
            ...
            f_df = work_df[work_df["type"].isin(["Walk/Exercise", "Walk", "Exercise"])].copy()
        elif activity_type == "All 3 Combined":
...
def render_github_codeforces_heatmap(df, activity_type="All 3 Combined", year="Last 365 Days"):
    ...
    if activity_type == "Test":
        ...
        colorscale = [
            [0.00, "#1e293b"], [0.01, "#1e1b4b"], [0.25, "#4338ca"],
            [0.55, "#6366f1"], [0.80, "#818cf8"], [1.00, "#c7d2fe"],
        ]
        bar_title = "hrs (Walk/Ex)"
    else:
