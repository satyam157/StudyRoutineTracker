...
def ask_llm(prompt):
    """Bridge to the main AI service"""
    return ai.get_ai_insight(prompt)

def generate_personalized_study_plan(user_prompt, quiz_analysis, days_left=None, exam_date=None, exam_type="UPSC"):
    """
    Generate a highly personalized study plan using the ULTIMATE_PROMPT_TEMPLATE.
    """
    prompt = ULTIMATE_PROMPT_TEMPLATE + f"""

    STUDENT DATA:
    - User query: {user_prompt}
    - Days until exam: {days_left if exam_date else 'Not specified'}
    - Exam type: {exam_type}
...
def generate_performance_summary(quiz_analysis):
    """
    Generate a data-driven performance analysis using the PERFORMANCE_SUMMARY_PROMPT.
    """
