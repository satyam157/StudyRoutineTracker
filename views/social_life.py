...
def render(USER, USER_CONFIG):
    ...
    # WentOutside, Turf, Travelling, Office, Coaching, Entertainment, Test (D-Day Exam)
    sm_df = df[
        (df['type'].isin(['WentOutside', 'Turf', 'Travelling', 'Office', 'Coaching', 'Entertainment'])) |
    ...
    _SM_COLOR_PRIORITY = {
        ...
        'Entertainment': ('#8b5cf6', '#ffffff'),   # Purple / Violet
    }
    ...
    for day in range(1, num_days + 1):
        ...
        for act in day_acts:
            ...
            if act_type == 'Travelling':
                ...
                if dur > 0 and amt > 0:
                    val = f"{format_duration(dur)} · ₹{amt:.0f}"
                elif dur > 0:
            ...
            if not date_acts.empty:
                ...
                for _, row in date_acts.iterrows():
                    ...
                    if row['type'] == 'Travelling':
                        ...
                        _dur = row.get('duration', 0) or 0
                        _amt = row.get('amount', 0) or 0
                        if _dur > 0 and _amt > 0:
                            _val = f"{format_duration(_dur)} | ₹{_amt:.0f}"
                        elif _dur > 0:
                            _val = format_duration(_dur)
                        elif _amt > 0:
                            _val = f"₹{_amt:.0f}"
                        else:
                            _val = ""
